...
Begin VB.Form FormDocument 
   ...
   Begin VB.Menu MenuEdit 
      ...
      Begin VB.Menu MenuEditBar0 
         Caption         =   "-"
      End
      Begin VB.Menu MenuEditBringFront 
         Caption         =   "置前(&R)"
      End
      Begin VB.Menu MenuEditBringBack 
         Caption         =   "置后(&B)"
      End
   ...
   Begin VB.Menu MenuPopup 
      ...
      Begin VB.Menu MenuPopupDelete 
         Caption         =   "删除(&D)"
      End
      Begin VB.Menu MenuPopupBar0 
         Caption         =   "-"
      End
      Begin VB.Menu MenuPopupBringFront 
         Caption         =   "置前(&F)"
      End
      Begin VB.Menu MenuPopupBringBack 
         Caption         =   "置后(&B)"
      End
      Begin VB.Menu MenuPopupBar1 
         Caption         =   "-"
      End
      Begin VB.Menu MenuPopupScreenCenter 
         Caption         =   "设为屏幕中心(&E)"
      End
      Begin VB.Menu MenuPopupBar2 
         Caption         =   "-"
      End
      Begin VB.Menu MenuPopupProperties 
         Caption         =   "对象属性(&R)..."
      End
   End
End
...
Public Sub RefreshWindow()
    ...
    With Documents(DocumentIndex)
        ...
        DrawStyle = 5
        FillStyle = 0
        For I = UBound(.Objects) - 1 To 0 Step -1
            If .Objects(I).Style <> osNull Then
                FillColor = .Objects(I).FillColor
                X = (.Objects(I).X - .OffsetX) * .Zoom + ScaleWidth / 2
                Y = (.Objects(I).Y - .OffsetY) * .Zoom + ScaleHeight / 2
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Flag = True
    For I = 0 To 3
        If Documents(DocumentIndex).FileName = RecentFiles(I) Then Flag = False
    Next
    If Flag Then
        RecentFiles(3) = RecentFiles(2)
        RecentFiles(2) = RecentFiles(1)
        RecentFiles(1) = RecentFiles(0)
        RecentFiles(0) = Documents(DocumentIndex).FileName
    End If
...
Private Sub Form_DblClick()
    ...
    With Documents(DocumentIndex)
        ...
        For I = 0 To UBound(.Objects) - 1
            If Sqr((MouseX - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (MouseY - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2) <= Min And Sqr((MouseX - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (MouseY - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2) <= .Objects(I).Radius * .Zoom + 45 Then
                J = I
                Min = Sqr((MouseX - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (MouseY - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2)
            End If
        Next
        If J >= 0 Then
...
Private Sub Form_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Button = 1 Then
        With Documents(DocumentIndex)
            DeltaX = X / .Zoom + .OffsetX - DragLastX
            DeltaY = Y / .Zoom + .OffsetY - DragLastY
            For I = 0 To UBound(.Objects) - 1
    ...
    If Button = 2 Then
        With Documents(DocumentIndex)
            ...
            Min = 1.79769313486231E+308
            For I = 0 To UBound(.Objects) - 1
                If Sqr((X - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (Y - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2) <= Min And Sqr((X - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (Y - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2) <= .Objects(I).Radius * .Zoom + 45 Then
                    J = I
                    Min = Sqr((X - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (Y - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2)
                End If
            Next
            If J >= 0 Then
                If Not .Objects(J).Selected Then
                    ...
                    .Objects(J).Selected = True
                End If
                PopupMenu MenuPopup, , , , MenuPopupProperties
            End If
        End With
    End If
    UpdateEnabled
...
Private Sub Form_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Button = 1 Then
        With Documents(DocumentIndex)
            DragLastX = X / .Zoom + .OffsetX
            DragLastY = Y / .Zoom + .OffsetY
            J = -1
            Min = 1.79769313486231E+308
            For I = 0 To UBound(.Objects) - 1
                If Sqr((X - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (Y - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2) <= Min And Sqr((X - (.Objects(I).X - .OffsetX) * .Zoom - ScaleWidth / 2) ^ 2 + (Y - (.Objects(I).Y - .OffsetY) * .Zoom - ScaleHeight / 2) ^ 2) <= .Objects(I).Radius * .Zoom + 45 Then
...
Private Sub MenuEditBringFront_Click()
    Dim I As Long
    Dim J As Long
    Dim AObject As TObject
    For I = 0 To UBound(Documents(DocumentIndex).Objects) - 1
        If Documents(DocumentIndex).Objects(I).Selected Then
            If Documents(DocumentIndex).Lock = I Then Documents(DocumentIndex).Lock = 0
            AObject = Documents(DocumentIndex).Objects(I)
            For J = I To 1 Step -1
                Documents(DocumentIndex).Objects(J) = Documents(DocumentIndex).Objects(J - 1)
            Next
            Documents(DocumentIndex).Objects(0) = AObject
        End If
    Next
    RefreshWindow
End Sub

Private Sub MenuEditBringBack_Click()
    Dim I As Long
    Dim J As Long
    Dim AObject As TObject
    For I = 0 To UBound(Documents(DocumentIndex).Objects) - 1
        If Documents(DocumentIndex).Objects(I).Selected Then
            If Documents(DocumentIndex).Lock = I Then Documents(DocumentIndex).Lock = UBound(Documents(DocumentIndex).Objects) - 1
            AObject = Documents(DocumentIndex).Objects(I)
            For J = I + 1 To UBound(Documents(DocumentIndex).Objects) - 1
                Documents(DocumentIndex).Objects(J - 1) = Documents(DocumentIndex).Objects(J)
            Next
            Documents(DocumentIndex).Objects(UBound(Documents(DocumentIndex).Objects) - 1) = AObject
        End If
    Next
    RefreshWindow
End Sub
...
Private Sub MenuGalaxyInsertObject_Click()
    ...
    With AObject
        ...
        .Style = osObject
        .Selected = True
        .X = Documents(DocumentIndex).OffsetX
        .Y = Documents(DocumentIndex).OffsetY
        .VX = 0
        .VY = 0
        .Mass = 1
        .Radius = 1
...
Private Sub MenuRunStart_Click()
    ...
    If Not Documents(DocumentIndex).Break Then
        ...
        Documents(DocumentIndex).InitLock = Documents(DocumentIndex).Lock
        Documents(DocumentIndex).InitZoom = Documents(DocumentIndex).Zoom
        Documents(DocumentIndex).InitOffsetX = Documents(DocumentIndex).OffsetX
        Documents(DocumentIndex).InitOffsetY = Documents(DocumentIndex).OffsetY
...
Private Sub MenuRunEnd_Click()
    ...
    Documents(DocumentIndex).Lock = Documents(DocumentIndex).InitLock
    Documents(DocumentIndex).Zoom = Documents(DocumentIndex).InitZoom
    Documents(DocumentIndex).OffsetX = Documents(DocumentIndex).InitOffsetX
    Documents(DocumentIndex).OffsetY = Documents(DocumentIndex).InitOffsetY
...
Private Sub MenuPopupBringFront_Click()
    Dim I As Long
    Dim J As Long
    Dim AObject As TObject
    For I = 0 To UBound(Documents(DocumentIndex).Objects) - 1
...
Private Sub MenuPopupBringBack_Click()
    Dim I As Long
    Dim J As Long
    Dim AObject As TObject
    For I = 0 To UBound(Documents(DocumentIndex).Objects) - 1
...
Private Sub MenuPopupScreenCenter_Click()
    With Documents(DocumentIndex)
        .OffsetX = (MouseX - ScaleWidth / 2) / .Zoom + .OffsetX
        .OffsetY = (MouseY - ScaleHeight / 2) / .Zoom + .OffsetY
    End With
    RefreshWindow
End Sub

Private Sub MenuPopupProperties_Click()
    Dim I As Long
    Dim PropertiesBox As FormProperties
    For I = 0 To UBound(Documents(DocumentIndex).Objects)
        If I = UBound(Documents(DocumentIndex).Objects) Then Exit Sub
